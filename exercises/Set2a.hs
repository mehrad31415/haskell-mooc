...
takeFinal n xs = if (length xs) < n
                     then xs
                     else drop (length xs - n) xs -- else let ys = reverse xs in reverse (take n ys)
...
palindromify s =  if (isPalindrome s) == True 
                      then s 
                      else 
                        if length s - 2 < 0 then "" else palindromify (take (length s - 2) (drop 1 s))
