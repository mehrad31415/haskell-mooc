...
capitalize xs = unwords( zipWith (:) (map (toUpper.head) (words xs)) (map tail (words xs)))
...
{-
solution 1 : --powers k max = [k^i| i<-[0..max], k^i<max]
got runtime error
-}
{-
solution 2 :
powers k max = filter (f k) [1..max]
...
-}
powers :: Int -> Int -> [Int]
powers n max = takeWhile (<= max) [n^i|i<-[0..max]]
...
whileRight check x = if isLeft (check x) then (left0 (check x)) else whileRight check (right0 (check x))
...
joinToLength a xs = [ x++y | x <- xs, y <- xs, length (x++y)==a ]
...
multiCompose fs x = multiComp (reverse fs) x 

multiComp [] x = x 
multiComp (f:fs) x = multiComp fs (f x)
------------------------------------------------------------------------------
