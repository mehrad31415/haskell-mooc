...
-- I did not want to use the hint in this question!

isOrdered :: Ord a => Tree a -> Bool
isOrdered Empty                    = True
isOrdered (Node x y z)  
  | y == Empty && z == Empty       = True 
  | y == Empty && x <= atRoot z    = isOrdered z
  | y == Empty && x > atRoot z     = False
  | atRoot y <= x && z == Empty    = isOrdered y 
  | atRoot y > x && z == Empty     = False
  | x >= atRoot y && x <= atRoot z = isOrdered y && isOrdered z
  | otherwise                      = False

atRoot :: (Ord a) => Tree a -> a
atRoot (Node x y z) = x
...
walk [] (Node r y z) = Just r
walk (x:xs) (Node r y z)
  | x == StepL = walk xs y
  | x == StepR = walk xs z
...
set [] val (Node r y z) = Node val y z 
set (x:xs) val (Node r y z)
  | x == StepL = Node r (set xs val y) z
  | x == StepR = Node r y (set xs val z)
...
search val t
  | inTree val t = Just (search' val t)
  | otherwise    = Nothing

inTree :: (Eq a) => a -> Tree a -> Bool
inTree _ Empty = False
inTree val (Node x y z)
  | val == x  = True
  | otherwise = inTree val y || inTree val z

search' :: Eq a => a -> Tree a -> [Step]
search' val (Node x y z)
  | val == x = []
  | inTree val y = StepL: search' val y
  | inTree val z = StepR: search' val z