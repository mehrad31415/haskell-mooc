...
prettyPrint' size [] (x2,y2) = if y2 == size then ".\n" else "." 
prettyPrint' size ((x1,y1):xs) (x2,y2)
  | (x1,y1) == (x2,y2) = if y2 == size then "Q\n" else "Q" 
  | otherwise          = prettyPrint' size xs (x2,y2)

-- concat == foldr (++) "" 
-- '.\n' is wrong. ['a','b'] = "ab" concat["a","b"]="ab"
...
prettyPrint2 size xs = concat [prettyPrint2' size xs (x,y) | x<- [1..size], y<-[1..size]]

prettyPrint2' :: Size -> Stack -> Coord -> String 
prettyPrint2' size xs (x,y)
